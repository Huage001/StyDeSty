...
def diff_argmax(logit, dim=-1):
    y_soft = logit.softmax(dim)
    index = y_soft.max(dim, keepdim=True)[1]
    y_hard = torch.zeros_like(logit, memory_format=torch.legacy_contiguous_format).scatter_(dim, index, 1.0)
    return y_hard - y_soft.detach() + y_soft


def likelihood(mu, log_var, tgt_embed):
    return ((mu - tgt_embed) ** 2 / log_var.exp() - log_var).mean()


def club(mu, log_var, tgt_embed):
...
def conditional_mmd_rbf(source, target, label, num_class):
    loss = 0
